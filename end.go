...
func endConversationWorker(jobs <-chan EndConversationEvent) {
	for e := range jobs {
		...
		if u.MatchChatID.Valid {
			...
			telegram.SendMessage(u.MatchChatID.Int64, "Your chat's ended! Start a new one! :D head to @unichatbotchannel for announcements/updates/ways to support us!", emptyOpts)
		...
		telegram.SendMessage(u.ChatID, "Your conversation is over, I hope you enjoyed it :) head to @unichatbotchannel for announcements/updates/ways to support development!", emptyOpts)
